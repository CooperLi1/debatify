...
export function Bar() {
  ...
  useEffect(() => {
    async function fetchUser() {
      ...
      setUserData(data); // Set the user data when fetched
    }
  ...
  const getEmailPrefix = (email: string) => {
    const parts = email.split('@'); // Split the email at the '@' character
    return parts[0]; // Return the first part, which is the prefix before '@'
  };
  
  var prefix = ''
  if(userData){
    prefix = getEmailPrefix(userData.user.email)
  }else{
    prefix = 'Guest'
  }
        ...
        <ul className="space-y-2 font-medium pt-8">
          <li>
              <a className="flex items-center p-2 text-gray-900 rounded-lg dark:text-white group">
                  ...
                  <span className="ms-3">{prefix}</span>
              </a>
          </li>
          <Search/>
          <Bookmarks/>
